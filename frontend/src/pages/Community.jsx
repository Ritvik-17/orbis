...
import { Plus, Bell, Search, UserPlus } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div className="lg:col-span-3">
          ...
          {loading ? (
            ...
            <div className="space-y-6">
              {posts.map(post => (
                <PostCard key={post.id} post={post} onUpdate={fetchCommunityData} />
              ))}
            </div>
          ) : (
        ...
        <div className="lg:col-span-1 hidden lg:block">
          ...
          <div className="bg-white rounded-xl shadow-sm border p-5 sticky top-32">
            ...
            <h2 className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mb-4">Your Friends</h2>
            {friends.length > 0 ? (
              <ul className="space-y-4">
                {friends.map(friend => (
                  <li key={friend.id} className="flex items-center justify-between group">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <Link to={`/profile/${friend.id}`} className="font-bold text-sm text-gray-900 block hover:underline tracking-tight">
                          {friend.username}
                        </Link>
                        <Link to={`/inbox?user=${friend.id}`} className="text-[9px] text-gray-400 font-bold uppercase tracking-widest hover:text-black transition-colors">
                          Message
                        </Link>
                      </div>
                    </div>
            ...
            <div className="mt-8 pt-6 border-t px-1">
                ...
                <div className="space-y-3 bg-gray-50/50 p-2 rounded-xl border border-gray-100 mb-4 max-h-48 overflow-y-auto">
                  {searchResults.map(u => (
                    <div key={u.id} className="flex items-center justify-between">
                      <Link to={`/profile/${u.id}`} className="flex items-center space-x-2 flex-1">
                        <UserAvatar 
                          user={u} 
                          className="w-6 h-6"
                        />
                        <span className="text-xs font-medium truncate w-24">{u.username}</span>
                      </Link>
                      <button 
                        onClick={async () => {
                          try {
              ...
              <p className="text-[10px] text-gray-400 italic">Search for event enthusiasts and organizers to expand your network.</p>
            </div>
          </div>
        </div>
      </div>
